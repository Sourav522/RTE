...
svg1.width=window.innerHeight*1.778;
svg1.height=window.innerHeight;
...
class nodeClass{
    constructor(nodeMap){
        ...
        this.nodeL3=[[1,2,3],[1,2],[1]];
        this.nodeL3ClickState=[[1,2,3],[1,2],[1]];
        this.nodeL3Line=[[1,2,3],[1,2],[1]];
        for(i=0;i<this.l2Length;i++){
            ...
            this.nodeL2[i].style.width='20vh';
            this.nodeL2[i].style.height='20vh';

            //connecterline drawing from Level 1 to Level2
            this.nodeL2Line[i] = document.createElementNS('http://www.w3.org/2000/svg','line');
            newLineDraw(this.nodeL2Line[i],78.85+10,40+10,parseFloat(this.nodeL2[i].style.marginLeft.replace('vh',''))+10,70+10);
            this.nodeL2Line[i].style.strokeOpacity=0.0;
            ...
            for(j=0;j<this.l3Length[i];j++){
                ...
                this.nodeL3[i][j].style.width='20vh';
                this.nodeL3[i][j].style.height='12vh';

                //connecterline drawing from Level 1 to Level2
            this.nodeL3Line[i][j] = document.createElementNS('http://www.w3.org/2000/svg','line');
            newLineDraw(this.nodeL3Line[i][j],parseFloat(this.nodeL2[i].style.marginLeft.replace('vh',''))+10,parseFloat(this.nodeL2[i].style.marginTop.replace('vh',''))+10,parseFloat(this.nodeL3[i][j].style.marginLeft.replace('vh',''))+10,110);
            this.nodeL3Line[i][j].style.strokeOpacity=0.0;
            }
        }
...
nodeMap=[[1,2,3],[1,2,3,4,5,6],[1]];

node = new nodeClass(nodeMap);

nodeHTMLDataFinal=[['00.svg','01.svg','02.svg'],['10.svg','11.svg','12.svg','13.svg','14.svg','15.svg'],['20.svg']];
nodeHTMLDataInitial=[['<div>Rays parallel to principal axis</div>','<div>Rays falling perpendicular on mirror</div>','<div>Rays coming from focus</div>'],['<div>object is between F and P</div>','<div>object is at F</div>','<div>object is between C and F</div>','<div>object is at C</div>','<div>object is beyond C</div>','<div>object is at Infinity</div>'],['<div>Applications of Concave mirror</div>']];
node.nodeL1.innerHTML='<div>Image Formation by Concave Mirror</div>';
node.nodeL2[0].innerHTML='<div>Special Rays</div>';
node.nodeL2[1].innerHTML='<div>Special Positions</div>';
node.nodeL2[2].innerHTML='<div>Applications</div>';
node.nodeL3[0][0].innerHTML='<div>Rays parallel to principal axis</div>';
node.nodeL3[0][1].innerHTML='<div>Rays falling perpendicular on mirror</div>';
node.nodeL3[0][2].innerHTML='<div>Rays coming from focus</div>';
node.nodeL3[1][0].innerHTML='<div>object is between F and P</div>';
node.nodeL3[1][1].innerHTML='<div>object is at F</div>';
node.nodeL3[1][2].innerHTML='<div>object is between C and F</div>';
node.nodeL3[1][3].innerHTML='<div>object is at C</div>';
node.nodeL3[1][4].innerHTML='<div>object is beyond C</div>';
node.nodeL3[1][5].innerHTML='<div>object is at Infinity</div>';
node.nodeL3[2][0].innerHTML='<div>Applications of Concave mirror</div>';
...
node.nodeL1.onclick=function(){
    if(node.nodeL1ClickState%2==1){
        ...
        for(i=0;i<node.l2Length;i++){
            ...
            node.nodeL2Line[i].style.strokeOpacity=1;
        ...
        for(i=0;i<node.l2Length;i++){
            ...
            node.nodeL2Line[i].style.strokeOpacity=0;
            for(j=0;j<node.l3Length[i];j++){
                node.nodeL3[i][j].style.opacity=0;
            }
        }
    ...
    node.nodeL2[i].onclick=function(){
        if(node.nodeL1ClickState%2==0){
            ...
            if(node.nodeL2ClickState[tempIndex]%2==1){
                ...
                node.nodeL2Line[tempIndex].style.strokeOpacity=1;
                ...
                for(j=0;j<node.l2Length;j++){
                    if(j!=tempIndex){
                        ...
                        node.nodeL2Line[j].style.strokeOpacity=0.2;
                        for(k=0;k<node.nodeL3[j].length;k++){
                            node.nodeL3[j][k].style.opacity=0;
                            node.nodeL3Line[j][k].style.strokeOpacity=0;
                ...
                for(k=0;k<node.nodeL3[tempIndex].length;k++){
                    ...
                    node.nodeL3Line[tempIndex][k].style.strokeOpacity=1;
                ...
                for(j=0;j<node.l2Length;j++){
                    if(j!=tempIndex){
                        ...
                        node.nodeL2Line[j].style.strokeOpacity=1;
                        ;
                    }
                } 
                for(k=0;k<node.nodeL3[tempIndex].length;k++){
                    node.nodeL3[tempIndex][k].style.opacity=0;
                    node.nodeL3Line[tempIndex][k].style.strokeOpacity=0;
                }
            }
            ...
            if(node.nodeL1ClickState%2==0){
                ...
                if(node.nodeL2ClickState[tempIndex1]%2==0){
                    if(node.nodeL3ClickState[tempIndex1][tempIndex2]%2==1){
                        ...
                        node.nodeL3Line[tempIndex1][tempIndex2].style.strokeOpacity=1;
                        node.nodeL3[tempIndex1][tempIndex2].style.transform='scale(8)';
                        node.nodeL3[tempIndex1][tempIndex2].style.zIndex='1';
                        node.nodeL2[tempIndex1].style.transform='scale(0.5)';
                        this.innerHTML='<image src="Image/'+nodeHTMLDataFinal[tempIndex1][tempIndex2]+'" width="100%">';
                            for(q=0;q<node.nodeL3[tempIndex1].length;q++){
                                if(q!=tempIndex2){
                                    node.nodeL3ClickState[tempIndex1][q]=1;
                                    //node.nodeL3[tempIndex1][q].style.opacity=0.5;
                                    node.nodeL3Line[tempIndex1][q].style.strokeOpacity=0.3;
                        ...
                        for(q=0;q<node.nodeL3[tempIndex1].length;q++){
                            ...
                            if(q!=tempIndex2){
                                ...
                                node.nodeL3Line[tempIndex1][q].style.strokeOpacity=1;
...
function newLineDraw(newLine,startX,startY,endX,endY){
    newLine.setAttribute('class','line');
    newLine.setAttribute('x1',window.innerHeight*((startX)/100).toString());
    newLine.setAttribute('y1',window.innerHeight*((startY)/100).toString());
    newLine.setAttribute('x2',window.innerHeight*((endX)/100).toString());
    newLine.setAttribute('y2',window.innerHeight*((endY)/100).toString());
